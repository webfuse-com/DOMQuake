import { type Event, type MutationEvent, type DOMQuakeOptions, EventArgument } from "./types.ts";
...
export class DOMQuake extends EventEmitter<Event> {
	...
	private computeWeight(record: MutationRecord): number {
		const typeFactor: number | undefined = MUTATION_WEIGHTS[record.type];
	...
	private tick() {
		...
		if(isAboveEntryThreshold && this.state !== "transition") {
			...
			this.emit<EventArgument>(this.state, {
